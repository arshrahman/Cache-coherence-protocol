...
class Simulation:
    ...
    def execute(self):
        ...
        while not all(done):
            ...
            for c in self.cores:
                #print('instuction num: ', c.current_instruction, ' core cycle ', c.stall_cycle, ' cache cycle ', c.cache.stall_cycle, 'snoop cycle ', self.snooping.stall_cycle)
                if done[c.core_num]:
                    c.instruction_type[COUNT] = self.counter
                    continue
                ...
                if not snooping_busy or not c.cache.is_generate_bus(instr_type, data):
                    stall_cyle = c.cache.process_data(instr_type, data)
                    if stall_cyle > 0:
                        c.stall_instruction()
                    else:
                        c.instruction_type[instr_type] += 1
    ...
    def results(self):
        ...
        for i in range(TOTAL_CORES):
            print('Core ', i)
            print('Total cycles: ', self.cores[i].instruction_type[COUNT])
